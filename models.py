...
class Linearformer(nn.Module):
    ...
    def forward(self, x):
        ...
        assert len(x.shape) == 3 and x.shape[1:] == (1, self.d_model)
...
def get_model_dataset(config):
    full_dataset = Dataset(func=config['func'],C=config['C'],diff_vocab=config['diff_vocab'],eqn_sign=config['eqn_sign'], device=config["device"])
    model = Transformer(
