...
When $c$ is the product of primes $p_1 \cdot p_2 \cdot p_3 ... \cdot p_n$, the algorithm computes $a \cdot b \text{ (mod } p_i)$ using a primitive root $g_i$ for all $1 \le i \le n$ and combines the solutions using the Chinese Remainder Theorem (CRT).
